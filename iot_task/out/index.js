...
var wsclient = new websocket_1.client();
...
async function sendDataToFont(deviceId, data, userId) {
    //use websocket
}
//device auth
async function deviceLogin(msgJson) {
    //
    mysqlConnection.query('insert into logs_devices_auth (device_id,randstr,serials,oper_count,md5,login_ip) values(?,?,?,?,?,?)', [msgJson.id, msgJson.randstr, msgJson.serials, msgJson.oper_count, msgJson.md5, msgJson.login_ip], (error, results, fields) => {
        if (error) {
            console.error('deviceAuth..insert mysql error');
            console.error(msgJson);
        }
    });
    const deviceFamilyList = await bssDatabases.listDocuments(conf_1.config.BSS_DEVICE_FAMILY_COLLETION_ID, [
        node_appwrite_1.Query.equal('device_id', msgJson.id)
    ]);
    for (var i = 0; i < deviceFamilyList.documents.length; i++) {
        const document = deviceFamilyList.documents[i];
        await redisClient.HSET(conf_1.config.REDIS_DEVICE_FAMILY_KEY, msgJson.id, document.family_id);
        const family = await redisClient.HGET(conf_1.config.REDIS_FAMILY_KEY, document.family_id);
        if (family == undefined || family === null || '' === family) {
            //insert family
            const familyDoc = await bssDatabases.getDocument(conf_1.config.BSS_FAMILY_COLLETION_ID, document.family_id);
            let fid = familyDoc.$id;
            redisClient.HSET(conf_1.config.REDIS_FAMILY_KEY, document.family_id);
            //TODO 
        }
    }
}
//user auth
function userLogin(msgJson) {
    mysqlConnection.query('insert into logs_user_auth (user_id,session_id,login_ip) values(?,?,?)', [msgJson.user_id, msgJson.session_id, msgJson.login_ip], (error, results, fields) => {
        if (error) {
...
//device msg
async function deviceMsg(msgJson) {
    var id = msgJson.id;
    //获取设备绑定的用户
...
//
async function deviceLogout(msgJson) {
    //更新redis 数据库
    //更新appwirte状态，通知关联用户
}
async function userLogout(msgJson) {
}
async function start_websocket() {
    ...
    wsclient.connect(conf_1.config.font_host_ws, 'echo-protocol');
}
async function main() {
