...
static esp_err_t get_device_id_handler(uint32_t session_id, const uint8_t *inbuf, ssize_t inlen,
                                        uint8_t **outbuf, ssize_t *outlen, void *priv_data){
    
    ESP_LOGI(TAG, "get_device_id_handler........");                                      
    nvs_handle_t my_handle;
    esp_err_t err = nvs_open("storage", NVS_READWRITE, &my_handle);
    uint64_t d_id = 0; // value will default to 0, if not set yet in NVS
    err = nvs_get_u64(my_handle, "id", &d_id);
    ...
    *outbuf = malloc(12); 
    ...
    uint8_t data[12];
    uint8_t *p_id_arr = (uint8_t *)&d_id;
    for(int i=0;i<8;i++){
        data[i] = p_id_arr[i];
    }
    uint32_t model = 0;
    nvs_get_u32(my_handle, "model",&model);
    uint8_t *p_model_arr = (uint8_t *)&model;
    for(int i=0;i<4;i++){
        data[i+8] = p_model_arr[i];
    }

    memcpy(*outbuf, data, 12);
    ESP_LOGI(TAG, "d_id........%lld",d_id);   
    ESP_LOGI(TAG, "d_model........%d",model); 
    ESP_LOGI(TAG, "get_device_id_handler..END......");      
...
static esp_err_t app_prov_start_service(void)
{
    ...
    if (g_prov->security == 0) {
        ...
        // protocomm_set_security(g_prov->pc, "proto-deviceid", &protocomm_security0, NULL);
    } else if (g_prov->security == 1) {
        protocomm_set_security(g_prov->pc, "prov-session", &protocomm_security1, g_prov->pop);
        // protocomm_set_security(g_prov->pc, "proto-deviceid", &protocomm_security1, g_prov->pop);
    }
    ...
    /* Add endpoint for provisioning to get esp info  */
    if (protocomm_add_endpoint(g_prov->pc, "proto-deviceid",
                               get_device_id_handler,
                               NULL) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set provisioning endpoint");
        protocomm_ble_stop(g_prov->pc);
        return ESP_FAIL;
...
static void app_prov_stop_service(void)
{
    ...
    protocomm_remove_endpoint(g_prov->pc, "proto-deviceid");
